...
export function SiteHeader() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-line bg-paper-50/80 backdrop-blur dark:bg-ink-950/80 py-2">
      ...
      {mobileOpen && (
        <div className="border-t border-line bg-paper-50 px-6 py-4 dark:bg-ink-950 md:hidden">
          <nav className="flex flex-col gap-4 text-sm text-ink-muted">
            {NAV_LINKS.map((link) => (
              <a key={link.href} href={link.href} onClick={() => setMobileOpen(false)} className="hover:text-ink">
                {link.label}
              </a>
            ))}
            <Link href="/login" onClick={() => setMobileOpen(false)} className="hover:text-ink">
              Log in
            </Link>
            <button
              onClick={toggleTheme}
